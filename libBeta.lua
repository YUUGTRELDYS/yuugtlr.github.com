...
local _splashEvent = Instance.new("BindableEvent")
local _splashDone = false
...
function YUUGTRL:CreateButton(parent, text, callback, color, position, size, gradientSeq)
    ...
    corner.Parent = btn
    local darker = btnColor and Color3.fromRGB(
    ...
    gradient.Parent = btn
    local brighter = btnColor and Color3.fromRGB(
    ...
    btn.MouseEnter:Connect(function()
        local hoverColor = btnColor and Color3.fromRGB(
...
function YUUGTRL:DarkenButton(button)
    ...
    if gradient then
        ...
        local darker = currentColor and Color3.fromRGB(
...
function YUUGTRL:RestoreButtonStyle(button, color)
    ...
    local gradient = button:FindFirstChildOfClass("UIGradient")
    if gradient then
        local darker = color and Color3.fromRGB(
        ...
        local brighter = color and Color3.fromRGB(
...
function YUUGTRL:CreateButtonToggle(parent, text, default, callback, position, size, colors)
    ...
    local function safeBrighten(c, amt)
    ...
    local function safeDarken(c, amt)
    ...
    local brighter = safeBrighten(buttonColor, 200)
    local function updateGradient()
        ...
        local darker2 = safeDarken(buttonColor, darkAmount)
        grad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, isOn and darker2 or buttonColor),
            ColorSequenceKeypoint.new(1, darker2)
        })
        if not IsEmojiOrSymbol(button.Text) then
            if isOn then
                button.TextColor3 = safeBrighten(buttonColor, 230)
            else
                button.TextColor3 = brighter
            end
        end
    ...
    button.MouseEnter:Connect(function()
        local hoverColor = safeBrighten(buttonColor, 30)
        local hoverDarker = safeDarken(hoverColor, 50)
        local grad = button:FindFirstChildOfClass("UIGradient")
        if grad then
            grad.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, hoverColor),
...
function YUUGTRL:CreateWindow(title, size, position, options)
    if not _splashDone then
        _splashEvent.Event:Wait()
    end
    options = options or {}
